...
form.onsubmit = (e) => {
    ...
    if (!eField.classList.contains("error") && !pField.classList.contains("error")) {
        ...
        window.location.href = "your-login-success-url.html";
    }
};
...
passToggleBtn.addEventListener("click", () => {
    if (pInput.type === "password") {
        pInput.type = "text";
        passToggleBtn.className = "bi bi-eye-slash icon";  // Change icon to eye-slash
    } else {
        pInput.type = "password";
        passToggleBtn.className = "bi bi-eye icon";  // Change icon to eye
    }
});